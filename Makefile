APK_PACKAGE := com.example.rustapp
LIB_NAME := rustapp # name of the native library
...
ANDROID_BUILD_TOOLS_DIR := $(ANDROID_HOME)/build-tools/34.0.0
...
%-signed.apk: %-unsigned.apk
	$(APKSIGNER) sign --ks release.key --out $@ $<
